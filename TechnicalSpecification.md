...
| state                         | yes        | `state=hkMVY7vjuN7xyLl5`                                   | Security code against false request attacks (cross-site request forgery, CSRF). `state` parameter is used to maintain state between the authentication and callback requests. `state` parameter should contain a hashed value of a string consisting of at least 16 random bytes and encoded in Base64. Read more about formation and verification of state under "7.3 Protection against false request attacks".                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| nonce                         | no         | `nonce=fsdsfwrerhtry3qeewq`                                | A unique parameter which helps to prevent replay attacks based on the OIDC protocol ([[OIDC-CORE](https://openid.net/specs/openid-connect-core-1_0.html)] "3.1.2.1. Authentication Request"). Can be used to associate and verify identity token with a specific authentication process. `nonce` parameter should contain a hashed value of a string consisting of at least 16 random bytes and encoded in Base64.                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
#### 6.1.4 Authentication termination response
...
- value of `nonce` if the parameter is used
...
2. a. If the client application does not buffer the public key, make request to public signature key endpoint and select the key corresponding to `kid` value received from JWT header. <br>
...
#### 7.1.2 Verifying the issuer of tokens
...
#### 7.1.3 Verifying the addressee of the tokens
...
#### 7.1.4 Verifying the validity of the tokens
...
#### 7.1.5 Verifying the eIDAS level of assurance
...
#### 7.1.6 Verifying nonce parameter

If the optional `nonce` parameter is used in the authentication request, the client should verify the `nonce` value in the identification token with the value generated by the client and submitted in the authentication request.
...
* Using `state` is compulsory and the parameter should be verified during [redirect request in the authentication flow](#61-authentication-request);
* Using `nonce` is optional and its purpose is to avoid identity token replay attacks. `nonce` should be stored on the server side and verification should be done during [identity token verification](#716-verifying-nonce-parameter);
